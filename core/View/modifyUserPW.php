...
if ($token->is_user_logged_in()) {
  ...
  if (isset($_POST["oldpassword"]) && isset($_POST["newpassword"])) {
    if ($customerC->Verify($_POST["oldpassword"], $_SESSION["email"])) {
      $hash = password_hash($_POST["newpassword"], PASSWORD_BCRYPT);
      $customerUpdt = new customer($_SESSION["user_id"], "", $hash, "", "", "");
                      ...
                      <div class="mj-column-per-100 mj-outlook-group-fix" style="font-size:0px;text-align:left;direction:ltr;display:inline-block;vertical-align:top;width:100%;">
                        <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="vertical-align:top;" width="100%">
                          <tbody><tr>
                            <td align="left" style="font-size:0px;padding:10px 25px;word-break:break-word;">
                              <div style="font-family:Helvetica, Arial, sans-serif;font-size:18px;font-weight:400;line-height:24px;text-align:left;color:#434245;">
                                <p style="margin: 0;">Hi, '.$customerC->getNAMEfromEMAIL($_SESSION['email']).'</p>
                              </div>
                            </td>
                          </tr>
                          <tr>
      ...
      header(
        "location: http://localhost/project2223_2a15-ninjahub/uservalidation/unsuccess.php?unsuccess=1"
      );
    }
  } else {
    header(
