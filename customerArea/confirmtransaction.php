        ...
        <section class="section">
            <div style="margin: auto;" class="col-lg-8">
                <div class="row">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h5 style="text-align: center;" class="card-title">Process Payment</h5>
                            <?php
              if (!$transactionC->checkCartExistPerUser($_SESSION['user_id']))
                echo('No Existing Items in Cart!');
              else{
                $res = $cartC->listcart($cartC->seekCartID($_SESSION['user_id']));
                echo("<table class='table table-hover'><th></th><th></th><th>Fee of Service</th>");
                $s = 0.0;
                while($row = $res->fetch()){
                  echo "<tr><td>".$row['DescServ']."</td><td>".$row['DurationServ']."</td><td>".$row['PriceServ']." TND</td></tr>";
                  $s += $row['PriceServ'];
                }
                }
                echo "<tr><td></td><td></td><td><b>TOTAL : ".$s." TND</b></td></tr>";
                if ($onlinepayC->checkCardExists($transactionC->retrieveTransactionID($transactionC->seekCartID($_SESSION['user_id']))))
                echo("<tr><td>PayPal Transaction ID: <b>".$onlinepayC->retrievePayPalTransactionID($onlinepayC->getTransactionID($_SESSION['user_id']))."</b></td></tr>");
                  else
                echo("<tr><td>Saved Payment Method is <b>Offline</b></td></tr>");
                echo("</table><br>");
              ...
              ?>
                            ...
                            <button
                                onclick='location.href = "http://localhost/project2223_2a15-ninjahub/core/View/confirmpayment.php";'
                                class="btn btn-primary">Process Payment</button>&nbsp;
                            <?php if (!$onlinepayC->checkCardExists($transactionC->retrieveTransactionID($transactionC->seekCartID($_SESSION['user_id'])))){ ?>
                            <button
                                onclick='location.href = "http://localhost/project2223_2a15-ninjahub/core/View/deletepayment.php";'
                                class="btn btn-primary">Cancel Payment</button>&nbsp;
                            <button 
                                onclick='location.href = "http://localhost/project2223_2a15-ninjahub/customerArea/modifypayment.php";'
                                class="btn btn-primary">Modify Payment</button><br>
                             <?php }?>
                        </div>
                            </div>
                    </div>
