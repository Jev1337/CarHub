...
$listPlate = $serviceC->allPlateUser($_SESSION['user_id']);
$cart = null;
    ...
    isset($_POST["CarPlate"])
        ...
        else
        {
          if ($cartC->checkCartExistPerUser($_SESSION['user_id'])){
            if(!$cartC->existComboCartServ($cartC->seekCartID($_SESSION['user_id']), $_POST['IDServ'])){
              $cart = new cart($cartC->seekCartID($_SESSION['user_id']),$_POST['IDServ'],$_POST['CarPlate']);
              $cartC->createCart($cart,0);
              header('Location: Location: http://localhost/project2223_2a15-ninjahub/uservalidation/success.php?success=2');
            }else
            header('Location: http://localhost/project2223_2a15-ninjahub/uservalidation/unsuccess.php?unsuccess=2');
          }else{
            $cart = new cart('',$_POST['IDServ'],$_POST['CarPlate']);
            $cartC->createCart($cart,1);
          }
                        ...
                        <div class="card-body">
                            ...
                            <form method="post">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-select" name="IDServ" aria-label="Default select example">
                                            <option selected value="">Choose service</option>
                                            <?php
                                              foreach ($list as $service) {
                                            ?>
                                            <option value=<?= $service['IDServ']; ?>><?= $service['DescServ']; ?>
                                            </option>
                                            <?php
                                              }
                                             ?>
                                ...
                                <div class="row mb-3">
                                    <label for="inputEmail" class="col-sm-2 col-form-label">Car Plate</label>
                                    <div class="col-sm-10">
                                        <select class="form-select" name="CarPlate" aria-label="Default select example">
