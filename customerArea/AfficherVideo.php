...
include_once "../core/Controller/galleryvideo.php";
...
$adherentC=new galleryvideoC();

if (isset($_GET["search"]) && isset($_GET["page"])) {
  $list = $adherentC->allVideosPages(
    $_GET["page"] * 5,
    ($_GET["page"] + 1) * 5,
    $_GET["search"]
  );
  $max_page = sizeof($adherentC->allVideoss($_GET["search"]));
} elseif (isset($_GET["search"])) {
  $list = $adherentC->allVideosPages(0, 5, $_GET["search"]);
  $max_page = sizeof($adherentC->allVideoss($_GET["search"]));
  ...
  $max_page = sizeof($adherentC->affichervideo());
} else {
  $list = $adherentC->allVideosPages(0, 5, "");
  $max_page = sizeof($adherentC->affichervideo());
}
...
<input type="text" id="search_filter" placeholder="search...">
                            <button><a href="AfficherVideo.php" id="search_url" style="color:black">Search</a></button>
                            <script>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <?php
                    foreach($list as $galleryimage){
                 ?>
                                    <tr>
                                        <th scope="row"><?= $galleryimage["GalleryID"]?></th>
                                        <td><?= $galleryimage["nom"]?></td>
                                        <td><?= $galleryimage["description"]?></td>
                                        <td><?= $galleryimage["URLVideo"]?></td>
                                        <td><?= $galleryimage["ModelID"]?></td>
                                        <td><?= $galleryimage["duree"]?></td>
                                        <td><a href="ModifierVideo.php?GalleryID=<?= $galleryimage["GalleryID"]?>">✏️</a></td>
                                    </tr>
                                    <?php
                  }
                 ?>
