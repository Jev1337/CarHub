        ...
        <section class="section">
                ...
                <div class="col-lg-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="POST" action="../core/View/addComplaint.php">
                                <div class="row mb-3">
                                    <label for="inputText" class="col-sm-2 col-form-label">Title of your
                                        complaint</label>
                                    <div class="col-sm-10">
                                        <input required minlength="4" pattern="^(?!.*\.)(?=.*[a-zA-Z]).+$" name="title" type="text" class="form-control">
                                    </div>
                                </div>

                                <div class="row mb-3">
                                    <label for="inputNumber" class="col-sm-2 col-form-label">Attachments (If
                                        available)</label>
                                    <div class="col-sm-10">
                                ...
                                <div class="row mb-3">
                                    <label for="inputPassword" class="col-sm-2 col-form-label">Describe your
                                        issue</label>
                                    <div class="col-sm-10">
                                        <textarea required minlength="10" name="description" class="form-control" style="height: 100px"></textarea>
                                    </div>
                                </div>
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <input type="text" name="search" placeholder="Search" title="Enter search keyword" value="<?php if (isset($_GET['search'])) echo $_GET['search']; ?>">
                    <button type="submit" title="Search"><i class="bi bi-search"></i></button>
                    &nbsp;
                    <select name ="by" style='width: 300px !important;' class="form-select" aria-label="Default select example">
                      <option value="1" <?php if (isset($_GET['by']) && $_GET['by'] == '1') echo "selected"; ?>>Complaint Title</option>
                      <option value="2" <?php  if (isset($_GET['by']) && $_GET['by'] == '2') echo "selected";?>>Description</option>
                      <option value="3" <?php if (isset($_GET['by']) && $_GET['by'] == '3') echo "selected"; ?>>Exact ID</option>
                    </select>
                </div>
                </form>
                            <table id="complainttable" class="table table-borderless datatable">
                                <thead>
                                    <tr>
                                        <th scope="col">#</th>
                                ...
                                <tbody>
                                    ...
                                    if($complaintC->ExistSingleUser($_SESSION['user_id'])){
                                        if(isset($_GET['by']) && isset($_GET['search'])){
                                            $res = $complaintC->listSingleUserComplaints($_SESSION['user_id'], $_GET['search'], $_GET['by']);
                                        }else{
                                            $res = $complaintC->listSingleUserComplaints($_SESSION['user_id']);
                            }
                 while($row = $res->fetch()){
                   
                   echo ("<tr><th scope='row'><a href='#'>".$row['ID']."</a></th><td>".$row['Title']."</td><td><a href='#' class='text-primary'>".$row['Description']."</a></td>");
                   if($row['IsPending'] && !$row['IsRejected'])
                        echo("<td><span class='badge bg-warning'>Pending Review</span></td></tr>");
                   else if (!$row['IsRejected'])
                    echo("<td><span class='badge bg-success'>Solved</span></td></tr>");
                    else
                    echo (" <td><span class='badge bg-danger'>Rejected</span></td></tr>");
                }
